...
A scroll-driven 3D teardown. The page starts with a PC on a desk; scrolling
lifts it, strips the panels, pulls the board, removes the cooler, lifts the CPU
out, splits it edge-on into its layers, shows measured cache latency, closes it
back up, and moves on to the GPU — then drops into the void for downloads.
...
```
hero → case → panels → board → cooler → CPU out → CPU layers →
architecture → measured cache → reassemble → GPU → ENTER THE VOID
```

The entire cinematic is data: `frontend/src/scenes/timeline.ts` holds the
chapters, their scroll ranges and the camera keyframes. Retiming the film means
editing numbers in one file, not chasing effects through components.
...
- **Simulated**: the activity panel shown during the architecture chapter. The
  API returns `"simulated": true` and the UI labels the panel from that field,
  so it cannot quietly start presenting invented numbers as the viewer's own.
...
Pushing to `main` runs `.github/workflows/deploy.yml`: it typechecks, builds,
lints, validates the scene data, compiles the C helper, and imports the backend
— then deploys to Vercel. The deploy step needs a `VERCEL_TOKEN` repository
secret; without it the checks still run and the deploy step skips rather than
failing the build.

`vercel.json` builds the frontend and mounts the FastAPI app as a Python
serverless function via `api/index.py`, so `/api/*` works in production the
same way the Vite proxy serves it in development.
...
Scroll. That is the whole interface — the camera and every part position are
functions of scroll offset. `End` (or the Download button) jumps straight to
the downloads.

## Accessibility

- Motion toggle in the nav; `prefers-reduced-motion` is respected by default.
- Native keyboard scrolling works throughout; `End` jumps to the downloads.
- Overlay copy is ordinary DOM, not text rendered into the canvas, so it is
...
- Scroll position lives in a ref, not state. `useFrame` reads it and mutates
  meshes directly, so scrolling never triggers a React render. Only the text
  overlay re-renders, and only when the chapter actually changes.
- The scroll value is itself eased frame-by-frame, so a flicked trackpad
  becomes a glide instead of a jump.
- All three subjects (PC, CPU, GPU) stay mounted and are faded per act.
  Mounting them on scroll would rebuild geometry mid-motion and stutter exactly
...
- `material.transparent` is set once at declaration rather than toggled per
  ...
  and getting that wrong silently breaks every opacity animation (it did,
  once — every fade in the teardown was a no-op until this was found).

## Known limitations

- The PC geometry is representational, not dimensionally accurate.
- The CPU reassembles as part of the timeline; there is no free-explore mode
  any more (it was replaced by the cinematic).
